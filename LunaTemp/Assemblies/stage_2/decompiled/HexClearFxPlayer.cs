...
public class HexClearFxPlayer : MonoBehaviour
{
	...
	[SerializeField]
	private float clearTilesFxLifetimePadding = 0.2f;
	...
	public void PlayClearTilesFxAtPosition(MonoBehaviour coroutineRunner, HexPoolService poolService, Vector3 position, Color tintColor)
	{
		if (!(clearTilesFxPrefab == null) && !(coroutineRunner == null))
		{
			...
			if (!(spawnedFx == null))
			{
				...
				coroutineRunner.StartCoroutine(ReturnClearTilesFxAfterDelay(poolService, spawnedFx, fxLifetime));
			}
		}
	}
	...
	private static void TintParticleSystems(ParticleSystem rootFx, Color tintColor)
	{
		...
		foreach (ParticleSystem fxSystem in fxSystems)
		{
			if (!(fxSystem == null))
			{
