...
public class CountdownTimerPanel : MonoBehaviour
{
	...
	[SerializeField]
	[Range(0.05f, 0.95f)]
	private float lowTimeThresholdNormalized = 0.2f;

	[Header("Colors")]
	[SerializeField]
	private Color barFrameNormalColor = new Color(0.11f, 0.38f, 0.86f, 1f);

	[SerializeField]
	private Color barBackNormalColor = new Color(0.06f, 0.22f, 0.58f, 1f);

	[SerializeField]
	private Color barFillNormalColor = new Color(0.35f, 0.95f, 0.18f, 1f);

	[SerializeField]
	private Color iconFrameNormalColor = new Color(0.1f, 0.37f, 0.86f, 1f);

	[SerializeField]
	private Color iconBackNormalColor = new Color(0.98f, 0.99f, 1f, 1f);

	[SerializeField]
	private Color barFrameLowColor = new Color(0.93f, 0.12f, 0.12f, 1f);

	[SerializeField]
	private Color barBackLowColor = new Color(0.55f, 0.02f, 0.02f, 1f);

	[SerializeField]
	private Color barFillLowColor = new Color(0.95f, 0.17f, 0.17f, 1f);

	[SerializeField]
	private Color iconFrameLowColor = new Color(0.93f, 0.12f, 0.12f, 1f);

	[SerializeField]
	private Color iconBackLowColor = new Color(1f, 0.92f, 0.92f, 1f);

	[Header("Arrow")]
	[SerializeField]
	private float arrowStartAngle = 45f;

	[SerializeField]
	private float arrowEndAngle = -235f;

	[SerializeField]
	private Color arrowNormalColor = new Color(0.9f, 0.2f, 0.2f, 1f);

	[Header("Low Time FX")]
	[SerializeField]
	private float iconShakeDuration = 0.24f;

	[SerializeField]
	private float iconShakeStrength = 9f;

	[SerializeField]
	private int iconShakeVibrato = 18;

	[SerializeField]
	[Range(0.02f, 0.9f)]
	private float lowBackdropBlendRange = 0.16f;
	...
	[SerializeField]
	private Image iconBackImage;

	[SerializeField]
	private Image iconNubImage;
	...
	private void StartShake()
	{
		...
		{
			StopShake();
			iconShakeTween = iconRoot.DOShakeRotation(iconShakeDuration, new Vector3(0f, 0f, iconShakeStrength), iconShakeVibrato, 90f, false).SetLoops(-1, LoopType.Restart).SetEase(Ease.Linear);
		}
	}
