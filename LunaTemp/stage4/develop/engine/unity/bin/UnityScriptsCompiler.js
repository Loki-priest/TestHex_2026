...
if ( TRACE ) { TRACE( "HexDragger#EndDrag", this ); }
                ...
                this.StartSettleTween(targetPosition.$clone(), hasFloorUnderStack ? floor.v : null, !hasFloorUnderStack);
            },
            /*HexDragger.EndDrag end.*/

            /*HexDragger.StartSettleTween start.*/
            StartSettleTween: function (targetPosition, targetFloor, isReturnToPlayerSlot) {
...
if ( TRACE ) { TRACE( "HexFloor#init", this ); }
                ...
                this.originalColor = new pc.Color( 0.42, 0.55, 0.78, 1.0 );
...
if ( TRACE ) { TRACE( "HexFloor#Awake", this ); }
                ...
                this.SetDropHighlight$1(false, true);
...
if ( TRACE ) { TRACE( "HexFloor#SetDropHighlight", this ); }
                ...
                this.SetDropHighlight$1(enabled, false);
            },
            /*HexFloor.SetDropHighlight end.*/

            /*HexFloor.SetDropHighlight$1 start.*/
            SetDropHighlight$1: function (enabled, force) {
if ( TRACE ) { TRACE( "HexFloor#SetDropHighlight$1", this ); }

                if (!force && enabled === this.isDropHighlighted) {
                    return;
                }
            ...
            /*HexFloor.SetDropHighlight$1 end.*/
...
if ( TRACE ) { TRACE( "HexFloor#CacheHighlightRenderers", this ); }
                ...
                if (renderers == null || renderers.length === 0) {
                    renderers = this.GetComponentsInChildren(UnityEngine.Renderer);
                }
                if (renderers == null || renderers.length === 0) {
                    return;
                }
                    ...
                    while ($t.moveNext()) {
                        ...
                        if (!(sharedMaterial == null)) {
                            ...
                            if (sharedMaterial.HasProperty(HexFloor.BaseColorId)) {
                                ...
                                state.BaseColor = this.originalColor.$clone();
...
if ( TRACE ) { TRACE( "HexFloor#OnDrawGizmosSelected", this ); }
        ...
        overloads: {
            "SetDropHighlight(bool, bool)": "SetDropHighlight$1"
        }
    });
    /*HexFloor end.*/
...
if ( TRACE ) { TRACE( "HexTile#ApplyColor", this ); }
    ...
    Bridge.define("HexTransferAnimator", {
        ...
        statics: {
            methods: {
                /*HexTransferAnimator.GetTransferFlipRadius:static start.*/
                GetTransferFlipRadius: function (startPosition, targetPosition, direction, up) {
if ( TRACE ) { TRACE( "HexTransferAnimator#GetTransferFlipRadius", this ); }

                    var transferOffset = targetPosition.$clone().sub( startPosition );
                    var radius = Math.abs(transferOffset.dot( direction )) * 0.5;
                    if (radius > 0.0001) {
                        return radius;
                    }
                    return UnityEngine.Mathf.Max(0.0001, pc.Vec3.projectOnPlane( transferOffset, up ).length() * 0.5);
                },
                /*HexTransferAnimator.GetTransferFlipRadius:static end.*/
...
if ( TRACE ) { TRACE( "HexTransferAnimator#GetStackName", this ); }
        ...
        fields: {
            ...
            tileTransferArcHeightMultiplier: 0
        },
        ctors: {
            init: function () {
if ( TRACE ) { TRACE( "HexTransferAnimator#init", this ); }
                ...
                this.tileTransferArcHeightMultiplier = 1.0;
            }
        },
        methods: {
...
if ( TRACE ) { TRACE( "HexTransferAnimator#CreateTileTransferTween", this ); }
                ...
                var startPosition = tileTransform.position.$clone();
                var startRotation = tileTransform.rotation.$clone();
                var flipRadius = HexTransferAnimator.GetTransferFlipRadius(startPosition.$clone(), targetPosition.$clone(), direction.$clone(), up.$clone());
                var startPivotPoint = startPosition.$clone().add( direction.$clone().clone().scale( flipRadius ) );
                var targetPivotPoint = targetPosition.$clone().sub( direction.$clone().clone().scale( flipRadius ) );
                var sequence = DG.Tweening.DOTween.Sequence();
                if (startDelay > 0.0) {
                ...
                DG.Tweening.TweenSettingsExtensions.Append(sequence, DG.Tweening.TweenSettingsExtensions.SetEase$2(DG.Tweening.Core.TweenerCore$3(System.Single,System.Single,DG.Tweening.Plugins.Options.FloatOptions), DG.Tweening.DOTween.To$4(function () {
                    return 0.0;
                }, Bridge.fn.bind(this, function (angle) {
                    var t = Math.max(0, Math.min(1, angle / 180.0));
                    var f = angle * (0.0174532924);
                    var vector = new pc.Vec3().lerp( startPivotPoint, targetPivotPoint, t );
                    var vector2 = direction.$clone().clone().scale( ((0.0 - Math.cos(f)) * flipRadius) ).add( up.$clone().clone().scale( (Math.sin(f) * flipRadius * this.tileTransferArcHeightMultiplier) ) );
                    tileTransform.position = vector.$clone().add( vector2 );
                    tileTransform.rotation = new pc.Quat().setFromAxisAngle( edgeAxis, angle ).clone().mul( startRotation );
